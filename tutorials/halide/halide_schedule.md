# Halide调度策略Schedule
Schedule是Halide程序的另一个组成部分； 它指定了算法计算的结构，即指定存储分配和pipeline的各个阶段之间的相互关系。它还需要指定如何计算每个阶段。 
本部分将讲解以下schedules。每个schedule除了给出C语言的伪代码，更深入讲解了为何这个schedule能有优化效果，认识schedule的优化本质。
